...
from address_backend import settings
...
# def green_sms_sender(phone, msg):
#     try:
#         greenweburl = "http://api.greenweb.com.bd/api.php"
#         token = "b32c40d07064593e7576f55802b46b1d"
...
# def twilio_sms_sender(phone, msg):
...
# def sms_sender(phone, msg):
#     if settings.SMS_PROVIDER == 'green':
#         return green_sms_sender(phone, msg)
#     elif settings.SMS_PROVIDER == 'twilio':
#         return twilio_sms_sender(phone, msg)
#     else:
#         return None
...
def compress_image(image_field):
    img = PIL_Image.open(image_field)
    if img.mode != 'RGB':
        img = img.convert('RGB')

    # Define maximum size and quality for compression
    max_size = (300, 300)
    quality = 70 # ranges from 0 to 95 for JPEG and WebP formats

    # Resize and compress the image
    img.thumbnail(max_size, PIL_Image.LANCZOS)

    # Prepare a BytesIO object to save the image
    output = io.BytesIO()
    img.save(output, format='JPEG', quality=quality)  # Save in JPEG format
    # img.save(output, format='WEBP', quality=quality)  # Save in WEBP format

    # Wrap the BytesIO object with InMemoryUploadedFile for saving to the image field
    output.seek(0)
    compressed_image = InMemoryUploadedFile(output, 'ImageField', "%s.jpg" % image_field.name.split('.')[0],
